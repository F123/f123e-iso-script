...
configure_system() {
	...
	if [ "$sh" == "/bin/bash" ]; then
		...
		echo "exec fish" >> "$FI_DIR"/extra/.bashrc-root
		echo "exec fish" >> "$FI_DIR"/extra/.bashrc
	elif [ "$shell" == "tcsh" ]; then
		cp "$FI_DIR"/extra/{.tcshrc,.tcshrc.conf} "$ARCH"/root/
		cp "$FI_DIR"/extra/{.tcshrc,.tcshrc.conf} "$ARCH"/etc/skel/
	elif [ "$shell" == "mksh" ]; then
		cp "$FI_DIR"/extra/.mkshrc "$ARCH"/root/
		cp "$FI_DIR"/extra/.mkshrc "$ARCH"/etc/skel/
	fi

	cp "$FI_DIR"/extra/.bashrc-root "$ARCH"/root/.bashrc
	cp "$FI_DIR"/extra/.bashrc "$ARCH"/etc/skel/
