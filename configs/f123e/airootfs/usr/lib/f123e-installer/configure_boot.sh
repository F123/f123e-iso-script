...
syslinux_config() {
	...
	if "$UEFI" ; then
		...
		cp "$FI_DIR"/boot/loader/syslinux/syslinux_efi.cfg ${ARCH}${esp_mnt}/EFI/syslinux/syslinux.cfg
		cp "$FI_DIR"/boot/splash.png ${ARCH}${esp_mnt}/EFI/syslinux
		...
		cp "$FI_DIR"/boot/loader/syslinux/syslinux.cfg "$ARCH"/boot/syslinux/
		cp "$FI_DIR"/boot/splash.png "$ARCH"/boot/syslinux/) &> /dev/null &
