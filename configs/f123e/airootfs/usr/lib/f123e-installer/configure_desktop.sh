		...
		if [ "$ex" -gt "0" ]; then
			...
			if [ "$?" -eq "0" ]; then
				if [ "$GPU" == "$gr0" ]; then
			        ...
			        if (<"$FI_DIR"/etc/nvidia340.xx grep "$pci_id" &>/dev/null); then
					...
					elif (<"$FI_DIR"/etc/nvidia304.xx grep "$pci_id" &>/dev/null); then
...
config_env() {
	...
	cp -r "$FI_DIR"/extra/desktop/ttf-zekton-rg "$ARCH"/usr/share/fonts
	cp "$FI_DIR"/extra/.zshrc "$ARCH"/root/
	cp "$FI_DIR"/extra/.zshrc "$ARCH"/etc/skel/
	cp "$FI_DIR"/extra/desktop/arch-anywhere-icon.png "$ARCH"/root/.face
	cp "$FI_DIR"/extra/desktop/arch-anywhere-icon.png "$ARCH"/etc/skel/.face
	cp -r "$FI_DIR"/extra/desktop/{arch-anywhere-wallpaper.png,arch-anywhere-icon.png} "$ARCH"/usr/share/pixmaps

	if (grep "AA-Xfce" <<<"$config_DE" &>/dev/null); then
		for file in $(ls -A "$FI_DIR/extra/desktop/xfce4"); do
			cp -r "$FI_DIR/extra/desktop/xfce4/$file" "$ARCH"/root/
			cp -r "$FI_DIR/extra/desktop/xfce4/$file" "$ARCH"/etc/skel/
		done
		cp -r "$FI_DIR"/extra/desktop/arch-anywhere-wallpaper.png "$ARCH"/usr/share/backgrounds/xfce/
	...
	if (grep "AA-Openbox" <<<"$config_DE" &>/dev/null); then
		for file in $(ls -A "$FI_DIR/extra/desktop/openbox"); do
			cp -r "$FI_DIR/extra/desktop/openbox/$file" "$ARCH"/root/
			cp -r "$FI_DIR/extra/desktop/openbox/$file" "$ARCH"/etc/skel/
		done
		cp -r "$FI_DIR"/extra/desktop/Arc/openbox-3 "$ARCH"/usr/share/themes/Arc
		cp -r "$FI_DIR"/extra/desktop/Arc-Dark/openbox-3 "$ARCH"/usr/share/themes/Arc-Dark
		cp -r "$FI_DIR"/extra/desktop/Arc-Darker/openbox-3 "$ARCH"/usr/share/themes/Arc-Darker
		cp -r "$FI_DIR"/extra/desktop/obpower.sh "$ARCH"/usr/bin/obpower
